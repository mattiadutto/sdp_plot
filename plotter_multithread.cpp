...
#define THREADS 1   // Per lettura multi-thread dei file.
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
int main(int argc, char **argv)
{
    ...
    /*
    Dichiaro un mutex, per mutua esclusione in lettura, al thread passo nulla, dichiaro tutto globale potrei passare:
    - putatore al file
    - map_points
    - mutex
    - MAX_HEIGHT
    ...
    Ogni thread ha:
    - signal_name, y, x
    - iterator per ricerca del signal_name 
    */
    ...
    if (file_pointer.is_open())
    {
       ...
       std::thread t(readPosition);
            
            //thread_pool.push_back(std::thread(readPosition));
    ...
    cout << "Elapsed time [READING STRUCTURE FILE] in seconds: "
         << chrono::duration_cast<chrono::nanoseconds>(end_structure_file - start).count()
         << " nanosec";
#endif
    ...
    if (file_pointer.is_open())
    {
        ...
        thread_pool.clear();
        for (int i = 0; i < N_THREADS; i++){
//            thread_pool.__emplace_back(thread(readResult));
            std::thread t(readResult);
            thread_pool.push_back(std::move(t));
        }
        for (auto &t : thread_pool)
            t.join();
    ...
    cout << "Elapsed time [READING STRUCTURE FILE] in seconds: "
         << chrono::duration_cast<chrono::nanoseconds>(end_coverage_file - end_structure_file).count()
         << " nanosec";
#endif
